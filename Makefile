...
dist/blinky.pex: dist
	...
	#docker run -w /src --rm -it -v `pwd`:/src ubuntu /bin/bash -c "./setup.sh && pex -f . -v --python=python3 -r all_reqs.txt --build . -o $@ -e blinky.blinky.py"
	docker run -w /src --rm -it -v `pwd`:/src ubuntu /bin/bash -c "./setup.sh && python3 -m pip wheel -w . . && pex -f . -v --python=python3 -r all_reqs.txt . -o $@ -e blinky.blinky.py"
